    ...
    public static void main(String[] args) {
        ...
        if (args.length > 0) {
            ...
            try {
                ...
                // The toRecall value refers to the amount of attempts per generation plus the best overall stack and then we add room to store the previous generation
                // The 3 refers to the three integers to remember for each box; id, orientation, and local height
                stacks = new int[toRecall + GENERATIONS][readBoxes.size()][3];
			...
			failedBoxes.clear();
				...
				for (int oldGen = 0; oldGen < GENERATIONS; oldGen++){
				    stacks[toRecall + oldGen][i][0] = stacks[oldGen][i][0];
				    stacks[toRecall + oldGen][i][1] = stacks[oldGen][i][1];
				    stacks[toRecall + oldGen][i][2] = stacks[oldGen][i][2];
				}
                            }
                            ...
                            repeats++;
                            if(repeats < REPEATS){
				// Load in last generation with improvement
				for (int oldGen = 0; oldGen < GENERATIONS; oldGen++){
				    for(int i = 0; i < bestStackSize || i < t; i++){
					stacks[oldGen][i][0] = stacks[toRecall + oldGen][i][0];
					stacks[oldGen][i][1] = stacks[toRecall + oldGen][i][1];
					stacks[oldGen][i][2] = stacks[toRecall + oldGen][i][2];
				    }
				}
                                roomForImprovement = true;
                            }
